...
public class DataProcessor : MonoBehaviour
{
    ...
    public static void ProcessData<T>(T data) where T : ISyncData
    {
        ...
        if (data is InGameActions iga)
        {
            ...
            {
                MelonLogger.Msg($"extraIZ已{((bool)iga.BetterIZDataEnabled?"启用":"关闭")}");
    ...
    private static void ProcessGaoshuData<T>(string data, Func<string, IEnumerable<T>> parser, Action<T> creator)
    {
        try
        {
            var decompressed = DecompressString(data);
            foreach (var item in parser(decompressed)) creator(item);
        }
    ...
    private static void ProcessJsonData<T>(string data, Action<IEnumerable<T>> processor)
    {
        try
        {
            var items = JsonSerializer.Deserialize<IEnumerable<T>>(data);
            if (items != null) processor(items);
        }
        catch (Exception ex) when (ex is JsonException || ex is NotSupportedException)
        {
            MLogger.Error("布阵代码存在错误！");
